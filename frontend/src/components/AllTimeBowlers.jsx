...
const AllTimeBowlers = () => {
  ...
  const fetchBowlers = async () => {
    setLoading(true)
    try {
      const response = await api.get('/players/all-time-bowlers')
      const sorted = response.data.slice().sort((a, b) => b.wkts - a.wkts)
      setBowlers(sorted)
    } catch (e) {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ color: '#a78bfa', fontSize: 22, fontWeight: 700, letterSpacing: 2 }}>⏳ Loading All-Time Bowlers...</div>
    </div>
  )
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 14, background: 'linear-gradient(135deg,#1e1b4b,#0f0a23)', border: '1px solid #6d28d944', borderRadius: 16, padding: '12px 32px', marginBottom: 12, boxShadow: '0 0 40px #7c3aed22' }}>
            ...
            <span style={{ fontSize: 28, fontWeight: 900, letterSpacing: 2, background: 'linear-gradient(90deg,#8b5cf6,#a78bfa,#c4b5fd)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              ALL-TIME IPL BOWLERS
            </span>
            <span style={{ fontSize: 32 }}>🏆</span>
          </div>
          <div style={{ color: '#94a3b8', fontSize: 14, letterSpacing: 1 }}>TOP WICKET TAKERS (2008–2025)</div>
        </div>
                ...
                {bowlers.map((b, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Average */}
                      <td style={{ padding: '14px 18px', textAlign: 'center' }}>
                        <span style={{ color: '#38bdf8', fontWeight: 700, fontSize: 15 }}>{b.bowling_average ?? '—'}</span>
                      </td>
